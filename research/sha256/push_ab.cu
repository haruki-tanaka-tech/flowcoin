/* Push a,b to 0 — extended annealing with more epochs + higher temp */
#include <cstdint>
#include <cstdio>
#include <time.h>
...
__device__ int ab_diff(const uint32_t W[16],const uint32_t d[16]){
    ...
    return popcnt(a1^a2)+popcnt(b1^b2);
}

__global__ void anneal_ab(uint64_t seed,int steps,float t0,float t1,
    int* best,uint32_t* bd,uint32_t* bm,int n){
    ...
    for(int f=0;f<1+((int)(rng>>32)%3);f++){rng=rng*6364136223846793005ULL+1442695040888963407ULL;
    ...
    int cur=ab_diff(W,d);int lb=cur;
    uint32_t bdd[16],bww[16];for(int i=0;i<16;i++){bdd[i]=d[i];bww[i]=W[i];}
    ...
    for(int s=0;s<steps;s++){
        ...
        if(ts<lb){lb=ts;for(int i=0;i<16;i++){bdd[i]=td[i];bww[i]=tW[i];}}
        temp*=decay;
    }
    int old=atomicMin(best,lb);
    if(lb<old){for(int i=0;i<16;i++){bd[i]=bdd[i];bm[i]=bww[i];}}
}

int main(){
    printf("Push a,b diff to 0 — extended search\n\n");
    int *d_best;uint32_t *d_d,*d_m;
    cudaMalloc(&d_best,4);cudaMalloc(&d_d,64);cudaMalloc(&d_m,64);
    int threads=1<<19,thermal=500,gb=64;
    for(int e=0;e<60;e++){
        struct timespec ts={0,thermal*1000000L};nanosleep(&ts,NULL);
        int h=64;cudaMemcpy(d_best,&h,4,cudaMemcpyHostToDevice);
        anneal_ab<<<threads/256,256>>>(e*104729ULL,8192,5000.0f+e*1000,0.001f,
            d_best,d_d,d_m,threads);
        cudaDeviceSynchronize();
        cudaMemcpy(&h,d_best,4,cudaMemcpyDeviceToHost);
        if(h<gb){gb=h;printf("  E%2d: a+b diff = %d/64 bits\n",e,h);}
        else if(e%10==0)printf("  E%2d: best=%d\n",e,gb);
        if(gb==0){printf("\n  *** a,b BOTH ZERO! ***\n");break;}
    }
    printf("\nFINAL: a+b diff = %d/64 bits\n",gb);
    if(gb<=4) printf("Birthday on remaining 6 regs: 2^%d = feasible!\n",(192+gb)/2);
    cudaFree(d_best);cudaFree(d_d);cudaFree(d_m);return 0;
}
